...
import {
  ensureTutorialDemoVehicle,
  seedTutorialPendingMission,
  seedTutorialSandboxMission,
} from "@/lib/tutorial-sandbox";
 ...
 * - `trackId: "missionApproval"`: ensures a PENDING sandbox mission so approvers can practice the decision (idempotent per org). No notification email is sent for sandbox missions.
 */
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  if (body.trackId === "missionApproval") {
    try {
      const r = seedTutorialPendingMission(db, organizationId);
      return NextResponse.json({
        mode: "missionApproval",
        missionId: r.missionId,
        sandboxMissionAlreadyExisted: r.alreadyExisted,
      });
    } catch (e) {
      console.error("[tutorial/seed] missionApproval", e);
      return NextResponse.json(
