...
export async function POST(req: NextRequest): Promise<NextResponse> {
  try {
    ...
    const { firebaseUid, email, firstName, lastName, name, role, department, organizationId, permissionLevel, isActive } =
      body;
    ...
    const existing = db
      .prepare("SELECT id FROM users WHERE firebase_uid = ? OR email = ?")
      .get(firebaseUid || "", email);
    ...
    `).run(
      ...
      db.prepare(`
      INSERT INTO users (id, firebase_uid, email, first_name, last_name, name, role, department, organization_id, permission_level, is_active, updated_at)
