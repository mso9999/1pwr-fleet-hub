...
| Submit a **vehicle request** | Users on the **EHS approved drivers** register for the org (`ehs_approved_drivers`, matched by email). Superadmin may bypass for testing. |
...
4. **Trip** — Operational checkout on `/trips` (vehicle, odometer, readiness). Not the same entity as a planned mission; optional future link via `missions.trip_id`.
